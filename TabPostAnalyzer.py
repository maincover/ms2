...
class PIProfileAnalyzer:
    ...
    def viewOriginalPlots(self, height = -1):
        ...
        if height < 0:
            f.set_size_inches(8,(len(self.dfs)-1)*0.1+0.004*np.sum([len(df) for df in self.dfs.values()]))
        else:
            f.set_size_inches(8, height)
        plt.tight_layout()
        f.savefig(os.path.join(self.path,self.saveName,'TA_'+ self.saveName+' colomap_by_files.png'))
        return f, axes
    ...
    def defineAlcoholTime(self,rankIndex = 0,aIndex = -1,more=10):
        ...
        if aIndex == -1:
            for i in range(more):
                if aIndex < int(self.all_fl_ts.columns[self.rate_of_change[i][1]]):
                    aIndex = int(self.all_fl_ts.columns[self.rate_of_change[i][1]])
            print 'autoDefineAlcohol is True, the alcohol index will be selected as:', aIndex
        ...
        print 'Alcohol time index is : ', self.alcoholIndex
    ...
    def outputPlot(self):
        ...
        f.set_size_inches(5,2*len(self.filenames))
        plt.tight_layout()
        f.savefig(os.path.join(self.path,self.saveName, 'PIP_'+ self.saveName+'_colomap_by_files.png'))
    ...
    def plotAllFiles(self,asp=0.02):
        ...
        plt.tight_layout()
        plt.gcf().savefig(os.path.join(self.path,self.saveName,'PIP_'+ self.saveName+'_colomap_all_files.png'))
    ...
    def saveDataToPickles(self):
        ...
        f = open(os.path.join(self.path,self.saveName, 'PIP_'+ self.saveName+'_df_relative_timegrid.pickle'), 'wb')
        pickle.dump(df_timegridrelative, f)
        self.df.to_pickle(os.path.join(self.path,self.saveName, 'PIP_'+ self.saveName+' _mortality_dataframe.pickle'))
        self.centroids_df.to_pickle(os.path.join(self.path,'PIP_'+ self.saveName, self.saveName+' _timeseries_dataframe.pickle'))
        analysis_metadata = {
        ...
        pickle.dump(analysis_metadata, open(os.path.join(self.path,'PIP_'+ self.saveName, self.saveName+' _analysis_metadata.pickle'),'w'))
...
class TabPostAnalyzer(PIProfileAnalyzer):
    ...
    def __init__(self, pth, filterFile="", group=1, debug=False, redefineInCells=True,autoDefineAlcohol=True):
        if pth[-1]=='/' or pth[-1]=='\\':
            pth = pth[:-1]
            print pth
        ...
        self.path = os.path.split(pth)[0]
        self.saveName = os.path.split(pth)[1]
        self.subfolderName = self.saveName
        ...
        self.autoDefineAlcohol = autoDefineAlcohol
        
    def getFilenames_abandoned(self):
    
        import glob
        existing_fns = glob.glob(os.path.join(self.path, self.subfolderName,'*.tab'))
        # get all the files in the path corresponding the filterFile.txt
        if self.filterFile != "":
            with open(self.filterFile, 'rU') as rf:
                previous = ''
                ...
                for line_fn in rf:
                    ...
                    if len(line_fn) > 0:
                        if os.path.join(self.path, line_fn) in existing_fns:
                        #and os.path.split(line_fn)[0]==self.subfolderName:
                            if previous == '':
                                count -= 1
                            if count == 0:
                                self.filenames.append(os.path.join(self.path, line_fn))
            ...
            self.filenames = glob.glob(os.path.join(self.path, self.subfolderName,'*.tab'))
            
        print "prefix name of pickles and pngs will be : ",'TA_'+  self.subfolderName+'-'+str(self.partial)
        
    def getFilenames(self):
        if len(self.filterFile):
            filterFile = self.filterFile
            name = self.subfolderName
            group_files = TabPostAnalyzer.groupByFilterFile(filterFile, name)
            self.filenames = group_files[self.partial-1]
            print "part, ",self.partial
    ...
    def viewOriginalPlots(self, height = -1):
        ...
        if height < 0:
            f.set_size_inches(12,len(self.dfs)*0.3+0.008*np.sum([len(df) for df in self.dfs.values()]))
        else:
            f.set_size_inches(12, height)
        plt.tight_layout()
        f.savefig(os.path.join(self.path,self.subfolderName, 'TA_'+ self.saveName+'_colomap_by_files.png'))
        return f, axes
    ...
    def plotAllFiles(self,asp=0.02):
        ...
        #plt.gcf().set_size_inches(10,30)
        plt.tight_layout()
        plt.gcf().savefig(os.path.join(self.path,self.subfolderName,'TA_'+ self.saveName+'_colomap_all_files.png'))
    ...
    def saveDataToPickles(self):
        ...
        #import Df_timegrid
        df_timegridrelative = Df_timegrid(self.df, self.relative_time_grid);
        f = open(os.path.join(self.path,self.subfolderName,'TA_'+  self.saveName+'_df_relative_timegrid.pickle'), 'wb')
        pickle.dump(df_timegridrelative, f)
        self.df.to_pickle(os.path.join(self.path,self.subfolderName, 'TA_'+ self.saveName+'_mortality_dataframe.pickle'))
        self.centroids_df.to_pickle(os.path.join(self.path,self.subfolderName, 'TA_'+ self.saveName+'_timeseries_dataframe.pickle'))
        analysis_metadata = {
            'time_grid' : self.time_grid,
            'time_indeces' : self.time_index,
            'background_threshold' : self.BACKGROUND_THRESHOLD,
        ...
        pickle.dump(analysis_metadata, open(os.path.join(self.path,self.subfolderName, 'TA_'+ self.saveName+'_analysis_metadata.pickle'),'w'))
